...
```
CHECK PROJECT DOCS (if ./docs/ and ./context/ exist):
- context/ → Existing PRD, business requirements, domain glossary
- docs/business/business-features/ → Feature list, feature status
- docs/knowledge-architecture/ → System architecture, service map
...
**Small** (≤ 150 lines): Single file `./plans/reports/requirements/REQ-{feature}.md`
**Large** (> 150 lines OR ≥ 4 sections): Folder `./plans/reports/requirements/{feature}/` → create `00-index.md` first, then each section `01-*.md`, `02-*.md` sequentially.
