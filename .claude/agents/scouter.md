...
```
1. CHECK PROJECT DOCS (if ./docs/ exists):
   - docs/knowledge-architecture/ → Already documented architecture (02-components.md)
   - context/ → Data models, API contracts, domain concepts
   - docs/knowledge-standards/ → Existing standards (01-code-style.md)
...
**Small** (≤ 150 lines): Single file `./plans/reports/scouts/SCOUT-{feature}.md`
**Large** (> 150 lines OR ≥ 4 sections): Folder `./plans/reports/scouts/{feature}/` → create `00-index.md` first, then each section `01-*.md`, `02-*.md` sequentially.
