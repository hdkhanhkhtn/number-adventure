...
**Location**: `./plans/reports/MAILBOX-{date}.md` — Append only, never overwrite.
...
| Phase / Team     | Output                                          |
| ---------------- | ----------------------------------------------- |
| Phase 1 (Disc.)  | `./plans/reports/brainstorms/BRAINSTORM-{task}` + `./plans/reports/scouts/SCOUT-{task}` |
| Phase 2 (Arch.)  | `./plans/reports/researchers/RESEARCH-{task}` + `./plans/reports/designs/ADR-{task}` |
| Phase 3 (Plan)   | `./plans/reports/plans/PLAN-{task}` (or `PLAN-{task}-phase1`, …) |
| Phase 4 (Review) | `./plans/reports/qa/QA-PLAN-{task}`                |
| ALL Phases        | `./plans/reports/MAILBOX-{date}.md`                  |

All files in `./plans/reports/` → English only. If plan has **> 3 phases** or **> 3 days** effort → produce **multiple plan files** (one per phase/milestone), each executable in sequence.
...
**Deliverable**: `./plans/reports/brainstorms/BRAINSTORM-{task}` + `./plans/reports/scouts/SCOUT-{task}`
...
**Deliverable**: `./plans/reports/researchers/RESEARCH-{task}` + `./plans/reports/designs/ADR-{task}`
...
**Deliverable**: `./plans/reports/plans/PLAN-{task}` (single) or `PLAN-{task}-phase1`, … (multi-phase)
...
**Deliverable**: `./plans/reports/qa/QA-PLAN-{task}` — includes: PASS/REVISE verdict, traceability matrix, security summary, business confirmation
...
## Deliverables
1. ✅ **Plan** — `./plans/reports/plans/PLAN-{task}` (or multi-phase files)
2. ✅ **QA** — `./plans/reports/qa/QA-PLAN-{task}` confirms PASS
3. 📬 **Debate** — `./plans/reports/MAILBOX-{date}.md`
