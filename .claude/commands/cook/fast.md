...
```
IF ./plans/reports/plans/PLAN-{feature} exists (.md file or folder):
  → READ and FOLLOW it
ELSE:
  → Create micro-plan inline
```

Any files in `./plans/reports/` or `./docs/` → English only.
...
| Prerequisite | READ `./plans/reports/plans/PLAN-{feature}` if exists                            |
