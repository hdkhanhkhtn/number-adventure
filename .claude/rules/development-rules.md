...
> **PREREQUISITE**: `CORE.md` must be loaded first. This file extends CORE with development-specific rules.
...
- **All code changes MUST go through feature branches → Pull Request → Review → Merge**
- **NEVER commit directly to `main` or `develop`** — create a feature branch first
- See `rules/git-workflow-rules.md` for complete git workflow protocol
- Use `git` skill for commit/push/PR operations
- Use `using-git-worktrees` skill for branch isolation
- Use `finishing-a-development-branch` skill when work is complete
...
- Run security scan for secrets before every commit (see `rules/git-workflow-rules.md` § 5)
