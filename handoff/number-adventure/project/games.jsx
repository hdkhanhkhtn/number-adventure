...
// Shared game chrome: top bar w/ close, hearts, progress, session timer
function GameHud({ hearts = 3, progress = 0, total = 5, onClose, sessionSecsLeft }) {
  const mins = sessionSecsLeft != null ? Math.floor(sessionSecsLeft / 60) : null;
  const secs = sessionSecsLeft != null ? sessionSecsLeft % 60 : null;
  const low = sessionSecsLeft != null && sessionSecsLeft < 60;
    ...
    <div style={{ padding: '56px 16px 10px', display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      {sessionSecsLeft != null && (
        <div style={{
          ...
          background: low ? '#FFDDE2' : '#FFF4DE',
          border: `2px solid ${low ? '#E88866' : '#C79528'}`,
          fontSize: 13, fontWeight: 700,
          color: low ? '#A33A1D' : '#7A4E0E',
          animation: low ? 'pulse-soft 0.8s ease-in-out infinite' : 'none',
        }}>
...
// —————————————————————————————————————————————————————————————
function HearTap({ onExit, onRoundEnd, config = {}, sessionSecsLeft }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0 }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, height: '100%', display: 'flex', flexDirection: 'column' }}>
        <GameHud hearts={hearts} progress={round} total={rounds} onClose={onExit} sessionSecsLeft={sessionSecsLeft}/>
...
// —————————————————————————————————————————————————————————————
function BuildNumber({ onExit, onRoundEnd, sessionSecsLeft }) {
  const rounds = 4;
  const [round, setRound] = React.useState(0);
  const [hearts, setHearts] = React.useState(3);
...
// —————————————————————————————————————————————————————————————
function EvenOdd({ onExit, onRoundEnd, sessionSecsLeft }) {
  const rounds = 5;
  const [round, setRound] = React.useState(0);
  const [hearts, setHearts] = React.useState(3);
...
// —————————————————————————————————————————————————————————————
function NumberOrder({ onExit, onRoundEnd, sessionSecsLeft }) {
  const rounds = 4;
  const [round, setRound] = React.useState(0);
  const [hearts, setHearts] = React.useState(3);
  const [picked, setPicked] = React.useState(null);
...
// —————————————————————————————————————————————————————————————
function AddTake({ onExit, onRoundEnd, sessionSecsLeft }) {
  const rounds = 4;
  const [round, setRound] = React.useState(0);
  const [hearts, setHearts] = React.useState(3);
  const [picked, setPicked] = React.useState(null);
